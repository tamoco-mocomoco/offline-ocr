---
title: "アプリのストア素材やプロモ動画をHTML+CSSで作り、Claudeで自動化した話"
emoji: "🎬"
type: "tech"
topics: ["playwright", "voicevox", "ffmpeg", "claude", "動画生成"]
published: false
---
...
.feature-card:nth-child(1) {
  animation: fadeSlideUp 0.6s ease forwards 4.8s;
}
.feature-card:nth-child(2) {
  animation: fadeSlideUp 0.6s ease forwards 9.5s;
}
.feature-card:nth-child(3) {
  animation: fadeSlideUp 0.6s ease forwards 18s;
}
...
const narrations = [
  {
    start: 1.5,
    file: "scene0-01",
    text: "オフラインOCR。どこにも通信しない、安全な範囲選択OCRです。",
  },
...
for (const item of narrations) {
  ...
  // 2. 音声合成
  const wav = await fetch(`${VOICEVOX_URL}/synthesis?speaker=${SPEAKER_ID}`, {
    method: "POST",
