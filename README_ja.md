...
- DEIM で領域検出 → XY-Cut 読み順 → PARSeq で文字認識 (複数行は同時 4 行まで並列化)
- 結果を `navigator.clipboard.writeText` で自動コピーし、ページ右下にトースト表示
- モデルは初回ロード後 IndexedDB にキャッシュされ、2回目以降は高速
- **画像ビューアー**: ローカル画像/クリップボード画像を開いて OCR (拡大 + 範囲選択も可)
- **PDF ビューアー**: ローカル PDF を開いてページ移動しながら任意ページを OCR (パスワード付きは非対応)
- **OCR 履歴**: コピー結果を `chrome.storage.local` に自動保存 (再コピー・編集・検索・削除)
- **クリーニングルール**: 正規表現ベースの整形ルールをカスタマイズ可能
- 表・レシート等の同一行検出でタブ区切り出力
- ふりがな (ルビ) の自動フィルタリング
- タイトな範囲選択・小画像・断片入りキャプチャに対する堅牢化 (v0.2 / v0.6 / v0.7.1)
