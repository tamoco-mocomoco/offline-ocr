...
- **Fixed a bug where OCR'ing a short Latin label on a dark background could return `the the the ...` or `.ES.ES.ES ...` style loops**. When PARSeq is uncertain about its input, the CTC decoder can degenerate into a loop at either the word level (identical whitespace-separated tokens) or the character level (a short substring repeating with no spaces). A post-processing step now detects both patterns (≥3 consecutive identical whitespace-separated tokens, or ≥3 consecutive repetitions of any 2–10 character substring) and trims everything from the loop start, applied uniformly to both the small-image bypass path and per-line DEIM path
