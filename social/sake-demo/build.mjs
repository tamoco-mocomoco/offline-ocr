/**
 * Build sake-demo short video.
 * Usage: node social/sake-demo/build.mjs
 */
...
function loadPlaywright() {
  for (const p of PLAYWRIGHT_PATHS) {
    try { return createRequire(p)("playwright").chromium; } catch {}
  }
...
async function recordHtml(htmlFile, outName, duration) {
  ...
  const fileUrl = `file://${resolve(__dirname, htmlFile)}`;
  ...
  const latest = files.map((f) => ({ name: f, time: statSync(resolve(tmpDir, f)).mtimeMs })).sort((a, b) => b.time - a.time)[0];
  if (latest) {
...
function toMp4(name) {
  ...
  execSync(`ffmpeg -y -i "${input}" -c:v libx264 -crf 18 -preset slow -pix_fmt yuv420p -an "${output}"`, { stdio: "pipe" });
...
function concatenate() {
  ...
  execSync(`ffmpeg -y -i "${intro}" -i "${scene}" -i "${outro}" -filter_complex "${filter}" -map "[vout]" -c:v libx264 -crf 18 -preset slow -pix_fmt yuv420p "${output}"`, { stdio: "pipe" });
...
function mergeAudio() {
  ...
  const audio = resolve(__dirname, "sake-narration.wav");
  const output = resolve(__dirname, "sake-demo-final.mp4");
  if (!existsSync(audio)) {
    console.log("  ⚠ sake-narration.wav not found.");
    execSync(`cp "${video}" "${output}"`, { stdio: "pipe" });
    return;
  }
  console.log("  Merging video + narration...");
  execSync(`ffmpeg -y -i "${video}" -i "${audio}" -c:v copy -c:a aac -b:a 192k -shortest "${output}"`, { stdio: "pipe" });
  const dur = execSync(`ffprobe -v error -show_entries format=duration -of csv=p=0 "${output}"`, { encoding: "utf8" }).trim();
  console.log(`    → sake-demo-final.mp4 (${parseFloat(dur).toFixed(1)}s)`);
}

async function main() {
  console.log("╔══════════════════════════════════════╗");
  console.log("║   Sake Demo — Short Video Builder    ║");
  console.log("╚══════════════════════════════════════╝\n");
  ...
  toMp4("intro"); toMp4("scene"); toMp4("outro");
  console.log("\n=== Step 3: Concatenating ===");
  concatenate();
  console.log("\n=== Step 4: Merging audio ===");
  mergeAudio();
  console.log("\n✓ Done! → social/sake-demo/sake-demo-final.mp4");
}
