/**
 * Generate narration for manga-demo short video.
 *
 * Usage: node social/manga-demo/generate-voice.mjs
...
const TOTAL_DURATION = 33; // イントロ4s + シーン15s + アウトロ13s + 余裕1s

const narrations = [
  // イントロ (0〜4s)
  { start: 0.0,  file: "mng-01", text: "Chrome拡張でOCR作ってみた！" },
  // シーン: 課題提示 (4〜9s)
  { start: 5.0,  file: "mng-02", text: "漫画の縦書きの吹き出しも" },
  // シーン: 範囲選択 (9〜13s)
  { start: 9.0,  file: "mng-03", text: "範囲をぎゅっと囲むだけで" },
  // シーン: 結果 (13〜17s)
  { start: 13.5, file: "mng-04", text: "横書きでサクッと取り出せるよ！" },
  // アウトロ (19〜32s)
  { start: 19.5, file: "mng-05", text: "しかも通信ゼロ。データは外に出ないよ" },
  { start: 23.0, file: "mng-06", text: "アプリじゃなくて、ブラウザの拡張機能ね" },
  { start: 26.0, file: "mng-07", text: "Chromeストアでオフラインオーシーアールって検索してね" },
];
...
async function main() {
  console.log("Generating manga-demo narration...\n");
  ...
  writeFileSync(resolve(outputDir, "manga-narration.wav"), fullWav);
  writeFileSync(resolve(outputDir, "manga-timestamps.txt"), timestamps.join("\n") + "\n");
  console.log(`    → manga-narration.wav (${TOTAL_DURATION}s)`);
  console.log(`    → manga-timestamps.txt`);
