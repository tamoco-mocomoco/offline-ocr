/**
 * Generate narration for haiku-demo short video.
 *
 * Usage: node social/haiku-demo/generate-voice.mjs
...
const narrations = [
  // イントロ (0〜4s)
  { start: 0.0,  file: "hk-01", text: "Chrome拡張でOCR作ってみた！" },
  // シーン: 題材提示 (4〜9s)
  { start: 5.0,  file: "hk-02", text: "崩し字で書かれた俳句も" },
  // シーン: 範囲選択 (9〜13s)
  { start: 9.0,  file: "hk-03", text: "ぎゅっと囲むだけで" },
  // シーン: 結果 (13〜17s)
  { start: 13.5, file: "hk-04", text: "ちゃんと読み取れちゃうよ！" },
  // アウトロ (19〜32s)
  { start: 19.5, file: "hk-05", text: "しかも通信ゼロ。データは外に出ないよ" },
  { start: 23.0, file: "hk-06", text: "アプリじゃなくて、ブラウザの拡張機能ね" },
  { start: 26.0, file: "hk-07", text: "Chromeストアでオフラインオーシーアールって検索してね" },
];
...
async function main() {
  console.log("Generating haiku-demo narration...\n");
  ...
  writeFileSync(resolve(outputDir, "haiku-narration.wav"), fullWav);
  writeFileSync(resolve(outputDir, "haiku-timestamps.txt"), timestamps.join("\n") + "\n");
  console.log(`    → haiku-narration.wav (${TOTAL_DURATION}s)`);
  console.log(`    → haiku-timestamps.txt`);
