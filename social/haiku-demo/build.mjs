/**
 * Build haiku-demo short video.
 *
 * 1. Record intro / scene / outro HTML as WebM via Playwright
 * 2. Convert to MP4
 * 3. Crossfade intro + scene + outro
 * 4. Merge with narration audio
 *
 * Usage: node social/haiku-demo/build.mjs
...
function mergeAudio() {
  ...
  const audio = resolve(__dirname, "haiku-narration.wav");
  const output = resolve(__dirname, "haiku-demo-final.mp4");

  if (!existsSync(audio)) {
    console.log("  ⚠ haiku-narration.wav not found. Copying video without audio.");
  ...
  console.log(`    → haiku-demo-final.mp4 (${parseFloat(dur).toFixed(1)}s)`);
}

async function main() {
  console.log("╔══════════════════════════════════════╗");
  console.log("║   Haiku Demo — Short Video Builder   ║");
  console.log("╚══════════════════════════════════════╝\n");
  ...
  console.log("\n✓ Done! → social/haiku-demo/haiku-demo-final.mp4");
}
